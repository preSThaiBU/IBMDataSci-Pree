...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        df=spacex_df[spacex_df['Launch Site'] == entered_site].groupby(['Launch Site', 'class']).size().reset_index(name='result')
        fig2=px.pie(df, 
        values='result',
        names='class',
...
def get_scatter_chart(entered_site,payload_slider):
    ...
    my_df = spacex_df[spacex_df['Payload Mass (kg)'].between(low,high)]
    if entered_site == 'ALL':
        fig3 = px.scatter(
            my_df, x="Payload Mass (kg)", y="class",
        ...
        new_df  = my_df[my_df['Launch Site'] == entered_site]
        fig4 = px.scatter(
            new_df, x="Payload Mass (kg)", y="class",
            color="Booster Version Category",
            size='Payload Mass (kg)',
            hover_data=['Payload Mass (kg)'],
